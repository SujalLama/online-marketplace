...
export default function EditShop ({match}) {
  ...
  useEffect(() => {
    ...
    read({
      ...
      if (data.error) {
        ...
        setValues({...values, id: data._id, name: data.name, description: data.description, owner: data.owner.name, image: data.image})
      }
    })
  ...
  const clickSubmit = () => {
    ...
    values.image && shopData.append('shop', values.image)
    update({
      shopId: match.params.shopId
    }, {
      t: jwt.token
